...
tempera <- subset(full_daily_weather, 
...
tapp <- tapply(tempera$T_DAILY_AVG, 
               tempera$WBANNO, function(x) {mean(x, na.rm = T)} )

dat <- stations[match(as.numeric(names(tapp)), stations$WBANNO),] 
dat$mar_temp <- tapp
```
